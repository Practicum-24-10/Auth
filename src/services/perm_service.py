...
from models.permissions import Permission, RolesPermissions
...
class RolePermissionService:
    ...
    def get_role_permission(cls, role_id, permission_id):
        return RolesPermissions.query.filter_by(
            role_id=role_id, permission_id=permission_id
        ).first()

    @classmethod
    def get_roles_permissions(cls, list_role):
        return Permission.join(RolesPermissions).join(Role).filter(Role.id.in_(list_role)).all()
